   ...
   void UpdateSelect()
   {
    ...
    private void Update()
    {
        if (canEase && EasyAnimation)
        {
            ...
            if(Mathf.Abs(value - this.easeValue) <= 0.001f)
            {
    ...
    void WrapContent(Vector2 value)
    {
        ...
        else 
        {
            ...
            for (int i = 0; i < reuse_item_count; i++)
            {
                ...
                pos = temp.localPosition;
                setMyActive(temp.gameObject, pos.y > min && pos.y < max && hasData);
