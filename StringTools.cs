...
using System.Text.RegularExpressions;
    ...
    public class StringTools
    {
        ...
        /// <param name="args"></param>
        /// <returns></returns>
        static public string FormatTable(string str, LuaInterface.LuaTable args)
        {
            string newStr = str;
            string rexStr = @"\{[0-9]+\}";
            MatchCollection mc = Regex.Matches(str, rexStr);
            if (mc.Count > 0)
            {
                for (int i = 0; i < mc.Count; i++)
                {
                    ...
                    if (i < args.Length)
                    {
                        newStr = newStr.Replace(subStr, args[i+1].ToString());
                    }
                    else
                    {
                        newStr = newStr.Replace(subStr, "");
        ...
        static public float[] SplitToFloat(string str, char splitStr)
        {
            string[] arr = SplitStr(str, splitStr);
            float[] list = new float[arr.Length];
            for(int i=0;i<arr.Length;i++)
            {
                list[i] = float.Parse(arr[i]);
            }
            return list;
        }
